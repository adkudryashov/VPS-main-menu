...
#
# Встроено в VPS-Server-Menu (Тесты -> Censorcheck), запускается локально.
# Поддерживаемые флаги:
#   -v, --verbose   подробный вывод
#   -d, --debug     отладочная информация в конце отчёта
#   -m, --mode      all (по умолчанию) | geoblock | dpi
#                     all      — доменные проверки + радар ТСПУ (RIPE Atlas)
#                     geoblock — только доступность сервисов (без RIPE Atlas)
#                     dpi      — только радар ТСПУ по сетям РФ (нужен RIPE_API_KEY)
# -----------------------------------------
...
# Режим проверки: all (всё сразу, поведение по умолчанию), geoblock (только доступность
# сервисов / гео-блоки), dpi (только радар ТСПУ по сетям РФ через RIPE Atlas).
# Значение можно передать флагом --mode / -m.
MODE="all"

# Позволяет задать RIPE_API_KEY (и другие переменные) снаружи, не трогая сам файл —
# это нужно, чтобы обновления скрипта (git pull) не затирали личный ключ пользователя.
# Файл создаётся автоматически из меню VPS-Server-Menu (Тесты -> Censorcheck).
CENSORCHECK_CONFIG="${CENSORCHECK_CONFIG:-/root/.vps-menu/censorcheck.env}"
[[ -f "$CENSORCHECK_CONFIG" ]] && source "$CENSORCHECK_CONFIG"
...
# Чекаем IP сервера и его ASN/org (нужно и для сводки, и для радара ТСПУ)
...
if [[ "$MODE" != "dpi" ]]; then
  ...
  echo "$LINE_SEP"
  echo -e "IP этого сервера: ${CYAN}${CURRENT_IP:-неизвестен}${RESET}${CURRENT_ASN:+ ${DIM}|${RESET} ${CYAN}$CURRENT_ASN${RESET}}"
fi

if [[ "$MODE" == "geoblock" ]]; then
  : # Радар ТСПУ по сетям РФ не входит в режим geoblock — пропускаем
elif [[ -z "$RIPE_API_KEY" || "$RIPE_API_KEY" == "Insert the key" ]]; then
  echo "$LINE_SEP"
  echo -e "${YELLOW}⚠ Радар ТСПУ (проверка доступности сервера из сетей РФ) пропущен:${RESET}"
  echo -e "${DIM}не задан личный RIPE Atlas API-ключ. Получить его можно на https://atlas.ripe.net/keys/${RESET}"
  echo -e "${DIM}и указать в меню VPS-Server-Menu: Тесты -> Censorcheck -> Настроить RIPE Atlas ключ.${RESET}"
